...
from .reachy_modified import Reachy
...
class JointLuos:
    ...
    def get_joint_velocities(self, names: List[str]) -> Optional[List[float]]:
        ...
        return self.reachy.get_joints_value(register='present_speed', joint_names=names)
        
        # pass
    
    def get_joint_efforts(self, names: List[str]) -> Optional[List[float]]:
        """Return the current effort of the specified joints."""
        return self.reachy.get_joints_value(register='present_load', joint_names=names)
        # pass
